...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🎵 Raw Suno API response:', JSON.stringify(rawData, null, 2));
    ...
    if (Array.isArray(rawData)) {
      ...
      console.error('❌ Unexpected Suno API response structure:', rawData);
    ...
    if (!firstTrack) {
      console.error('❌ First track is undefined. Data:', data);
    ...
    if (!firstTrack.audio_url) {
      console.warn('⚠️ Audio URL not immediately available, status:', firstTrack.status);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data && data.length > 0 && data[0].audio_url) {
          console.log(`✅ Audio URL ready after ${(i + 1) * 2} seconds`);
          return data[0].audio_url;
        }
      }
    } catch (error) {
      console.warn(`⚠️ Poll attempt ${i + 1} failed:`, error);
    }
  }
  
  console.error('❌ Failed to get audio URL after polling');
  return null;
}
